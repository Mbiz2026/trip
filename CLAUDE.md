...
渡航中、ユーザーはチャットで「今すぐ使える情報」をその場で聞いてくる(ラウンジ・シャトルバス・営業時間・店の場所・チェックイン締切など)。速度より確度が優先。以下は実際に事故った教訓:

- **WebSearchの合成要約だけで断定しない。** 「シャトルがある」「ラウンジで入れる」のような可否・場所・時間帯が絡む主張は、可能な限りWebFetchで一次情報(公式サイトの該当ページ)を直接開き、該当箇所を引用してから答える。急いで検索結果の要約だけで答えて、後から訂正する事態を繰り返さないこと
- **運行時間帯・landside/airsideの区別は特に事故りやすい。** 「ある/使える」だけでなく「何時から何時まで」「保安検査の前か後か」までセットで確認する。片方だけ確認すると、実際には使えない情報になる(例: あるシャトル便は日中は別ルートで、深夜だけ目的地に停まる、等)
- **固有名詞は空港/都市ごとに検証し直す。** 別の空港・別の都市で確認したラウンジ名・店名を、似た文脈だからと使い回さない
- **確信が持てない時は「確認できていません」とはっきり言う。** 曖昧な二次情報をそれらしく整形して断定的に出さない
- 上記を怠って現地で誤情報を出し訂正した実例は過去セッションのログにある。同じ種類の間違い(ターミナル取り違え、運行時間帯の見落とし、店の場所の未確認)を繰り返さないこと
