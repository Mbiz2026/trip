...
特定の渡航のために作った1回きりのページは `travel_tools/<地名>_<出発日YYYYMMDD>.html` に置く(`/tabi-shiori` が作るしおりは `shiori_` 接頭辞)。ツールではないのでPages配信はしない。上のルールは同じく全部適用する。

- `travel_tools/melbourne_20260923.html` — 「メルボルン弾丸 作戦司令室」(2026/9/23-26・大阪発)。6タブ: 🎯結論/✈️ルート比較(経由地10か所を安い順にカード化)/🃏裏技(海外発券・オープンジョー等を◎○×で正直に評価)/🗓旅程/🦘現地/📖出典。**この日程はAFLグランドファイナル週(9/25金はビクトリア州の祝日、9/26土が決勝)かつ日本側はシルバーウィーク最終日発**という特殊条件が結論を決めている。JSは「時間の値段」計算機のみ(静的な初期表示あり)
