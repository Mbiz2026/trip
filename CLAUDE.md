...
3. `travel_tools/flight_search_hq.html` — 「航空券サーチ司令塔」。行き先・日程・人数を入れると、Skyscanner・KAYAK・Google フライト・Trip.com の「安い順の検索結果ページ」へ直行する条件入りリンクをまとめて生成する(価格の取得はしない。アーティファクトとしても公開中)
