...
4. **推測で答えない。出す数字は実物で裏を取ってから書く。** 所要時間や価格といった間接的な数字から、実際の便名・発着時刻・到着日を逆算して書くのは禁止(過去に「所要11時間40分」から到着日を計算して行程表を書き、ユーザーに指摘された実績あり)。交通機関を案内するときは **(1)便名 (2)出発時刻と到着時刻 (3)到着が当日か翌日か (4)運航曜日** の4点を必ず実データで確認してから書く。確認できたものと推定したものは必ず区別し、取れなかったものは「未確認」と明記して確認方法をユーザーに示す。**「たぶんこうなるはず」で行程表を書かない。**
5. **ライト/ダークテーマ両対応。** 生成物は必ず両方で見た目を確認する
6. **生成後は表示確認をする。** ヘッドレスブラウザで(JS有り/無しの両方)スクリーンショットを撮ってから納品する。無い環境ではユーザーに確認を依頼する
7. **迷ったら止まってユーザーに聞く。** 情報の鮮度・法規制が絡む分野(渡航制度・料金など)は特に慎重に。断定できないことは「要確認」と明記する
...
- `travel_tools/shiori_bangkok_siemreap_20260923.html` — **2026年9月23〜27日 3カ国4都市周遊**(関空→ハノイ乗継→バンコク2泊→シェムリアップ1泊→ホーチミン乗継→関空)。10タブ構成: 行程と関門/事前手続き/アンコール/持ち物/薬と健康/電源と機内/お金と通信/ラウンジ/注意と緊急/出典と確認日。PWA自己完結(manifest・アイコンをdata URIで埋込)。Pagesでは `/shiori.html`(共有用の短いURL)と `/shiori-20260923.html`(保存用)の両方で配信
  - **調査ノート: `travel_tools/notes/shiori_20260923_sources.md`** — 外務省の危険情報・安全対策基礎データの引用、5便の時刻表の裏どり手順、ラウンジ一覧、アンコール情報などの一次情報の控え。**旅行中に質問が来たらまずここを読む**

しおりを新しく作るときの原則:
- **便の情報は必ず実データで裏を取る**(便名・出発到着時刻・到着が当日か翌日か・運航曜日)。時刻表サイトは古い値を持っていることがあるので、**公開時刻表と運航実績の2ソースで突き合わせる**(実例: VN320はFlightMapperが00:20発だったが、PlaneMapperの直近7日の実績はすべて00:10発だった)
- 公式ソースまで裏どりできなかったものは `<span class="badge warn">要確認</span>` / `二次情報` と明記し、**行程を左右する判断には使わない**
- **最後に「出典と確認日」タブを付ける**。旅行中に質問が来たときの回答根拠になる
- ドメインの乗っ取りに注意(実例: `cambodianembassy.jp` は2026年8月時点で大使館ではなく無関係のFXサイトになっていた)。リンクする前に必ず中身を開いて確認する
