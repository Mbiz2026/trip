...
6. `travel_tools/award_radar.html` — 「特典レーダー」。JAL特典航空券の**空席さがしの作業そのもの**を速くする1ファイル・オフライン動作アプリ。「時期は決まっている・行き先は空席次第」の探し方を前提に設計。5タブ構成: 🎯段取り(JAL公式「空席照会カレンダー」で行き先を決めずに探す手順・3周の巡回・巡回メモ保存・360日前の発売日逆算・どこかにマイル)/📋台帳(**中核機能**。路線×日付のマスをタップして未→×→△→○→◎で記録。localStorage保存・期間を前後にずらしても日付ひもづけで記録が残る・3日以上前のマスは点線で「調べ直し」の目印・控えの書き出し/読み込み)/🔗検索リンク(公式と提携社サイトの入口・日付形式の変換キット・空港コード早見)/🌐提携社ガイド(カンタスFlight Reward Finder=ログイン不要で12ヶ月俯瞰、BA、aa.com、キャセイ、アラスカ。「探すのはツール、取るのは公式」の原則と幻の空席の注意)/📖ルールと出典。**JALがマイル・空席をAPI等で公開していないため自動取得はしない**。記録するのはユーザーが自分の目で見た結果、という設計。JAL公式サイトはbot対策で本文を機械取得できない(403)ため、記述は検索結果に含まれる公式ページの記述と公式プレスリリースが根拠。二次情報は※表記。GitHub Pagesでは `/radar.html` として配信。アイコンは `travel_tools/dev/gen_radar_icons.py` で生成(冪等)
...
### パターンB: GitHub Pages常設ホスティング(`flight_finder.html` / `mile_compass.html` / `award_radar.html`)
...
- manifestとアイコンはアプリごとに分ける。司令塔は `manifest.webmanifest` + `icon-*.png`、特典レーダーは `radar.webmanifest` + `radar-icon-*.png`(共有すると、ホーム画面に追加したとき別アプリが起動してしまう)。アイコンは外部の画像生成を使わず `travel_tools/dev/gen_radar_icons.py` のようにHTML+SVGをヘッドレスブラウザで描画してPNG化する
- 新しいアプリを足したら pages.yml の `paths` と dist へのコピー、sw.js の `ASSETS` の3か所すべてに追加する
